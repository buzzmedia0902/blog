...
class FreeeInvoiceCreatorMCP {
  ...
  private freeeClient: FreeeClient;
  private subsidyClient: SubsidyClient;
  ...
  constructor() {
    ...
    this.subsidyClient = new SubsidyClient(
      this.config.subsidy.apiKey,
      this.config.subsidy.baseUrl
    );
  ...
  async createInvoice(
    ...
    try {
      ...
      if (request.customerId && !customerName) {
        const data = await this.subsidyClient.getCustomerWithApplication(
          request.customerId
        );
  ...
  async getCustomer(customerId: string) {
    try {
      const { customer, application } =
        await this.subsidyClient.getCustomerWithApplication(customerId);

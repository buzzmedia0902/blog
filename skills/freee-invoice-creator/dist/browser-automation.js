...
export class BrowserAutomation {
     ...
     * freee にログイン
     */
    async loginToFreee(email, password) {
        if (!this.page)
     ...
     * 請求書作成ページに移動
     */
    async navigateToInvoiceCreation() {
        if (!this.page)
            throw new Error("Browser not initialized");
        // 請求書メニューを表示
     ...
     * 取引先を検索・選択
     */
    async selectPartner(partnerName) {
        if (!this.page)
            throw new Error("Browser not initialized");
        // 取引先入力欄に入力
        const partnerInput = await this.page.locator('input[placeholder*="取引先"]');
    ...
    async fillInvoiceBasicInfo(data) {
        ...
        // 請求日を入力
        const invoiceDateInput = await this.page.locator('input[placeholder*="請求日"]');
        await invoiceDateInput.fill(data.invoiceDate);
        // 入金期日を入力
        const dueDateInput = await this.page.locator('input[placeholder*="入金期日"]');
        await dueDateInput.fill(data.dueDate);
        // 件名を入力
        const subjectInput = await this.page.locator('input[placeholder*="件名"]');
     ...
     * 自社担当者を選択
     */
    async selectCompanyRepresentative(representativeName) {
        if (!this.page)
            throw new Error("Browser not initialized");
        const repInput = await this.page.locator('input[placeholder*="担当者"]');
    ...
    async fillInvoiceDetails(data) {
        ...
        // 摘要を入力
        const descInput = await this.page.locator('input[placeholder*="摘要"]');
        await descInput.fill(data.description);
        // 数量を入力（特殊な手順が必要）
        const quantityInput = await this.page.locator('input[placeholder*="数量"]');
        ...
        // 単位を確認
        const unitSelect = await this.page.locator('select[name*="unit"]');
